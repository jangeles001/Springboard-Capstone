...
export async function updateOneMealLogEntryByUUID(mealUUID, logId){
    await MealLog.findOneAndUpdate({ mealUUID },
        {$set: { isDeleted: true, correctedFromLogId: logId}},
    )
    return;
}

export async function getAllMealLogsByUserPublicId(userPublicId){
   const mealLogs = MealLog.find({ userPublicId }).select(PRIVATE_FIELDS_EXCLUSIONS).lean();
